...
export default function SpecificArticlePage() {
    ...
    const fetchArticleData = async () => {
        try {
            ...
            console.error(`Erreur lors de la récuparation des données de l'article : ${error}`);
        }
    };
            ...
            <div className="w-full md:w-3xl my-10">
                <div className="flex flex-col justify-center space-y-5">
                    ...
                    >
                        ...
                        <Heart className="h-5 w-5 transition-transform group-hover:scale-125 text-red-500" />
                    </Button>
                    ...
                    <div className="article-content space-y-8">
                        {descChunks.map((chunk, index) => (
                            <div key={index} className="space-y-4">
                                {chunk.map((desc, i) => (
                                    <p key={i} className="text-color-secondary">{desc}</p>
                                ))}
                                {article.screens?.[index] && (
                                    <Image
                                        ...
                                        width={800}
                                        height={600}
                                        className={`object-cover w-full rounded-lg mx-auto h-full ${index % 2 ? 'max-w-[800px]': 'max-w-[500px]'}`} 
                    ...
                    <div className="flex justify-between">
                        <p className='font-thin'><span className='font-bold'>{article.views}</span> vues.</p>
                        <p className='font-thin flex space-x-1'>
                            <span className='font-bold'>{article.likes}</span> j&apos;aime{article.likes > 1 && 's'}.
